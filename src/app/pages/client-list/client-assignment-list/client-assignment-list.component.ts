...
@Component({
  ...
  styleUrls: ['./client-assignment-list.component.css'],
...
export class AssignmentListComponent implements OnInit {
  ...
  private executeGetReport(status: string) {
    return this.assignmentService.getAssignmentByClientId(status, this.clientId)
      .subscribe(result => {
  ...
  editAssignment(assignmentId: number, clientId: number) {
    this.router.navigate([`/edit-client-assignment/${assignmentId}/${clientId}`]);
  }
  viewAssignment(id: number, clientId: number) {
    this.router.navigate([`/view-client-assignment/${id}/${clientId}`]);
  }
