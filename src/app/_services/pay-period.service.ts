...
export class PayPeriodService {
  ...
  createPayPeriod(payPeriodRequest: PayPeriodRequest) {
    return this.http.post(this.baseurl + `/payperiod`, payPeriodRequest).pipe(timeout(this.timeoutInSeconds), catchError(this.handleError));
  }
  updatePayPeriod(id: number, payPeriodRequest: PayPeriodRequest) {
    return this.http.put(this.baseurl + `/payperiod/${id}`, payPeriodRequest).pipe(timeout(this.timeoutInSeconds), catchError(this.handleError));
  }
  deletePayPeriod(id: number) {
    return this.http.delete(this.baseurl + `/payperiod/${id}`).pipe(timeout(this.timeoutInSeconds), catchError(this.handleError));
  }
}
